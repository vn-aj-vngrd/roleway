...
Roleway Agent is account-configured through user-supplied provider credentials and can read permitted context across all owned Workspaces. It is a native conversational work surface rather than a generic chat widget. The product workflow, tool tiers, approval states, and rollout are specified in [`ROLEWAY-AGENT.md`](ROLEWAY-AGENT.md). A conversation is account-owned. Direct Agent entry can read across owned Workspaces; entry from Home, Inbox, Opportunities, Interviews, Contacts, or Documents starts with that Workspace and a validated page label. Workspace-scoped queries exclude other Workspaces on the server. An Opportunity focus carries its owning Workspace. The saved scope stays fixed for the conversation, including when expanded from floating chat into full Agent. Existing conversations retain their account-wide scope. Starting a new conversation creates a clean context boundary; proposed mutations still resolve one exact destination Workspace.

The chat route streams validated response text through the Vercel AI SDK and reports persisted context, model, and proposal-preparation steps. Streaming never executes proposed mutations; approval remains a separate server action. Interrupted streams retain their saved conversation and expose recovery. Within the signed-in app, conversation streams are owned by the account layout, so page navigation and conversation switching do not cancel them. Agent navigation returns to the last opened conversation; a spinner marks background work and a blue dot marks an unread completed response. Failed background requests use an attention indicator. Opening the corresponding conversation clears its unread state. Reloading or closing the browser still relies on the persisted-run recovery path.
