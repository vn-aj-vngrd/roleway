...
- The account avatar anchors the bottom of the sidebar. Its menu opens above the trigger, uses the shared floating-panel primitive, and enters with a restrained 120ms fade-and-rise. Default avatars use Waypoint Blue rather than a neutral fill. The sidebar avatar is 32px with 12px initials and 7px inner padding.
...
- Menus, select lists, date pickers, filters, saved views, and account popovers reuse one borderless floating-panel treatment and minimal entrance motion; reduced-motion preferences remove the animation. Icon-only search, create, and sidebar controls expose tooltips. Tooltips use the current canvas with foreground text and a muted boundary rather than black inverse bubbles in light mode. Both composed tooltips and legacy `data-tooltip` controls render through the shared portal, escape card and scroll-container clipping, and stay inside the viewport. Keyboard focus and Escape are supported; native dialogs keep tooltips in their active top layer.
...
The floating Agent reuses the full conversation's Markdown and run timeline in a bounded, scrollable panel. Keep context to one muted line, suggestions to two quiet starting points, and the composer to text plus icon actions. Stream replies in place; link to the persisted full conversation for recovery and proposal review. Closing the panel does not discard an active request. The sidebar account identity shows initials, name, and the current plan; email belongs in Profile settings.
