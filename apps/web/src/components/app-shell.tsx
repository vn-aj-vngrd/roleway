...
type EntitySearchResult = {
  kind: "opportunity" | "job" | "document" | "contact" | "interview" | "note";
  entity_id: string;
  title: string;
  subtitle: string;
  href: string;
...
const BreadcrumbContext = createContext<
  ((items: BreadcrumbItem[] | null) => void) | null
>(null);
...
export function CreateJobButton({
  ...
  return (
    <Button
      ...
      size={size}
      onClick={() =>
        window.dispatchEvent(new CustomEvent("roleway:create-job"))
      }
    >
      {children}
    </Button>
...
function routeBreadcrumbs(pathname: string): BreadcrumbItem[] {
  ...
  if (segments[0] === "inbox" && segments[1] === "new")
    return [{ label: "Job inbox", href: "/inbox" }, { label: "Add job" }];
  if (segments[0] === "interview" && segments[1] === "new")
    return [
  ...
  if (segments[0] === "settings") {
    ...
    if (
      ...
      return [
        ...
        { label: "Workspace", href: `/settings/workspaces/${segments[2]}` },
        { label: "General" },
      ];
    ...
    return segments[1]
      ? [
          { label: "Settings", href: "/settings/profile" },
          { label: settingsLabels[segments[1]] ?? "Settings" },
        ]
      : [{ label: "Settings" }];
  }
  const title =
...
const searchEntries: SearchEntry[] = [
  {
    href: "#create-job",
    label: "Add a job",
    description: "Capture a role in your inbox",
    keywords: "create new capture import",
    icon: Plus,
    shortcut: "C",
  },
  {
    ...
    label: "Add a contact",
    description: "Record a person and follow-up",
    keywords: "create person recruiter referral",
    icon: UserRound,
  },
  {
    href: "/interview?create=true",
    label: "Add an interview",
    description: "Schedule a conversation and preparation",
    keywords: "create calendar interview",
    icon: CalendarClock,
  },
  {
    href: "/documents?create=true",
    label: "Create a document",
    description: "Start application material",
    keywords: "new resume cover letter note",
    icon: FileText,
  },
  {
    href: "#create-workspace",
    label: "Create a workspace",
    description: "Separate a distinct target or strategy",
    keywords: "new project search strategy",
    icon: BriefcaseBusiness,
  },
  {
    href: "/home",
    label: "Home",
    description: "Workspace context, tasks, interviews, and next actions",
    keywords: "home due focus workspace overview",
    icon: LayoutDashboard,
  },
  {
    href: "/opportunities",
    label: "Opportunities",
    description: "Track every serious role and Next Action",
    keywords: "pipeline opportunities kanban board stages",
    icon: Target,
  },
  {
    href: "/inbox",
    label: "Inbox",
    description: "Review saved roles before tracking them",
    keywords: "jobs listings capture",
    icon: Inbox,
    shortcut: "I",
  },
  {
    href: "/interview",
    label: "Interviews",
    description: "Schedule and prepare conversations",
    keywords: "calendar preparation",
    icon: CalendarClock,
  },
  {
    href: "/contacts",
    label: "Contacts",
    description: "People, context, and follow-ups",
    keywords: "people recruiters interviewers referrals crm",
    icon: UserRound,
  },
  {
    href: "/documents",
    label: "Documents",
    description: "Resumes, cover letters, and notes",
    keywords: "files resume cover letter",
    icon: FileText,
  },
  {
    href: "/agent",
    label: "Agent",
    description: "Ask across workspaces and approve changes",
    keywords: "ai assistant questions drafts tasks",
    icon: Navigation,
  },
  {
    href: "/insights",
    label: "Insights",
    description: "Review performance across workspaces",
    keywords: "metrics conversion analytics",
    icon: ChartNoAxesColumnIncreasing,
  },
  {
    href: "/notifications",
    label: "Notifications",
    description: "Review important changes across Roleway",
  ...
  },
  {
    ...
    keywords: "account name settings",
    icon: UserRound,
  },
  {
    href: "/settings/workspaces",
    label: "Workspaces",
    description: "Manage separate search strategies and preferences",
    keywords: "projects settings compensation remote",
...
function WorkspaceRouteAction({ pathname }: { pathname: string }) {
  if (
    ...
    return (
      <CreateJobButton className="workspace-route-create">
  ...
  if (pathname === "/interview")
    return (
      <Link className="workspace-route-create" href="/interview?create=true">
        <Plus aria-hidden="true" />
        Add interview
      </Link>
    );
  if (pathname === "/contacts")
    return (
      <Link className="workspace-route-create" href="/contacts?create=true">
        <Plus aria-hidden="true" />
        Add contact
      </Link>
    );
  if (pathname === "/documents")
    return (
      <Link className="workspace-route-create" href="/documents?create=true">
        <Plus aria-hidden="true" />
        Create document
      </Link>
    );
      ...
      >
        <div className="command-input-wrap">
          ...
          <input
            ...
            aria-label="Search Roleway"
            placeholder="Search opportunities, people, documents…"
            value={query}
        ...
        >
          ...
          {results.map((entry, index) => {
            ...
            return (
              <button
                key={entry.resultKey ?? entry.href}
                role="option"
          ...
          {searchState === "error" ? (
            <div className="search-empty" role="status">
              ...
              <span>
                Navigation and actions still appear above. Try entity search
                again.
              </span>
            </div>
          ) : null}
          ...
          results.length === 0 ? (
            <div className="search-empty">
              ...
              <span>Try a company, role, person, document, note, or page.</span>
            </div>
          ) : null}
        </div>
        <footer className="command-footer">
          ...
          <span>Searching {projectName}</span>
        </footer>
      </div>
    </div>
  );
...
function AccountMenu({
  ...
  return (
    <div className="account-area" ref={menuRef}>
      {open ? (
        <div
          className="account-popover floating-panel"
          data-side="top"
          role="menu"
        ...
        >
          <div className="account-menu-group">
            ...
            <Link role="menuitem" href="/settings/workspaces">
              Workspaces
            </Link>
            <Link role="menuitem" href="/settings/notifications">
              Settings
            </Link>
          </div>
          <div className="account-menu-group">
...
export function AppShell({
  ...
  isAdmin: boolean;
  agentConnection: { id: string; label: string } | null;
}) {
  const pathname = usePathname();
  const router = useRouter();
  const [searchOpen, setSearchOpen] = useState(false);
  const [createJobOpen, setCreateJobOpen] = useState(false);
  const [createJobProjectId, setCreateJobProjectId] = useState(
    activeProject.id,
  );
  ...
  const closeCreateWorkspace = useCallback(() => {
    ...
    if (params.has("create") || params.has("workspaceError"))
  ...
  useEffect(() => {
    ...
    if (
      params.has("workspaceCreate") ||
      (pathname === "/settings/workspaces" && params.has("create"))
    )
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() =>
      mobileMoreRef.current?.querySelector<HTMLElement>("a, button")?.focus(),
    );
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") setMobileMoreOpen(false);
      if (event.key !== "Tab") return;
      const controls = Array.from(
        mobileMoreRef.current?.querySelectorAll<HTMLElement>(
  ...
  const isAccountWideRoute = ["/agent", "/insights", "/notifications"].some(
    (route) => pathname === route || pathname.startsWith(`${route}/`),
  );
    ...
    return (
      <BreadcrumbContext.Provider value={setCustomBreadcrumbs}>
        <div className="settings-app-shell">
          <aside
            className="settings-shell-sidebar"
            aria-label="Settings navigation"
          >
            <SettingsNav shell pathname={pathname} />
          </aside>
          <main className="settings-shell-main" id="main-content">
            {children}
          </main>
          ...
          {createWorkspaceOpen ? (
            <CreateModal
              ...
              description="Keep one job-search direction and its Opportunities together."
              context="New workspace"
              size="compact"
              hideContext
              onClose={closeCreateWorkspace}
            >
              <WorkspaceCreateForm onCancel={closeCreateWorkspace} />
            </CreateModal>
          ) : null}
          <ToastViewport />
        </div>
  ...
  return (
    <BreadcrumbContext.Provider value={setCustomBreadcrumbs}>
      <div
        className="app-shell"
        data-sidebar={compactSidebar ? "hidden" : "expanded"}
      >
        <aside
          className="sidebar"
          id="roleway-sidebar"
        ...
        >
          <div className="sidebar-brand-row">
            <Link
              ...
              href="/home"
              aria-label="Roleway home"
              data-tooltip="Roleway"
            >
            ...
            <div className="sidebar-quick-actions" data-tour="commands">
              <Tooltip>
                ...
                <TooltipContent side="bottom" align="start" sideOffset={8}>
                  Search workspace <kbd data-slot="kbd">⌘K</kbd>
                </TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger
                ...
                <TooltipContent side="bottom" align="start" sideOffset={8}>
                  Create job <kbd data-slot="kbd">C</kbd>
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
          <div className="sidebar-navigation">
            <nav className="nav-group sidebar-primary-nav" aria-label="Primary">
              {sidebarTopNav.map((item) => (
                <NavItem
                  key={item.href}
                  item={item}
                  pathname={pathname}
            ...
            >
              {(project, active) => (
                <nav
                  className="nav-group sidebar-workspace-navigation"
                  aria-label={`${project.name} workspace`}
                >
                  {[...workNav, ...searchNav].map((item) => (
                    <NavItem
                      key={item.href}
                      item={item}
        ...
        <main className="main" id="main-content">
          <header className="workspace-toolbar">
            <Tooltip>
              <TooltipTrigger
                render={
                  <button
                    ...
                    aria-label={
                      compactSidebar ? "Open sidebar" : "Close sidebar"
                    }
                    aria-expanded={!compactSidebar}
                    aria-controls="roleway-sidebar"
                  />
                }
              >
              ...
              <TooltipContent side="bottom" align="start" sideOffset={8}>
                {compactSidebar ? "Open" : "Close"} sidebar{" "}
            ...
            <nav className="workspace-breadcrumb" aria-label="Breadcrumb">
              ...
              {breadcrumbs.map((item, index) => (
                <span
                  className="workspace-breadcrumb-part"
            ...
            <div className="workspace-toolbar-actions">
              <WorkspaceRouteAction pathname={pathname} />
            </div>
          </header>
        ...
        {mobileMoreOpen ? (
          <div
            ...
            role="presentation"
            onMouseDown={(event) =>
              event.target === event.currentTarget && setMobileMoreOpen(false)
            }
          >
            <div
            ...
            >
              <header>
                <div>
                  <strong>More</strong>
                  <span>
                    {isAccountWideRoute ? "All workspaces" : activeProject.name}
                  </span>
                </div>
                <button
        ...
        {createJobOpen ? (
          <CreateModal
            title="Add a job"
            description="Capture the listing now. Decide whether it belongs in your pipeline after review."
            context="New job"
            identity={
              <JobWorkspacePicker
                projects={projects}
                value={createJobProjectId}
                onValueChange={setCreateJobProjectId}
              />
            }
            size="large"
          ...
          >
            <JobCreateForm
              projectId={createJobProjectId}
              projectName={
                projects.find((project) => project.id === createJobProjectId)
                  ?.name ?? activeProject.name
              }
              onCancel={closeCreateJob}
            />
          </CreateModal>
        ) : null}
        {createWorkspaceOpen ? (
