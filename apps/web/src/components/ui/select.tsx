...
function SelectValue({ className, ...props }: SelectPrimitive.Value.Props) {
  return (
    <SelectPrimitive.Value
      ...
      className={cn("flex min-w-0 flex-1 text-left leading-normal", className)}
      {...props}
    />
  )
}
...
function topmostOpenDialog() {
  ...
  return Array.from(document.querySelectorAll<HTMLDialogElement>("dialog[open]")).at(-1) ?? null
}
