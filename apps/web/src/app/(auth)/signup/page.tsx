...
export default async function SignupPage(props: {
  ...
  const { data: admission, error: admissionError } = await supabase.rpc(
    "signup_admission_status",
  );
  const productionUnavailable =
    process.env.NODE_ENV === "production" &&
    (admissionError || !process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY);
  const acceptingSignups =
    !productionUnavailable &&
    (admission?.acceptingSignups ?? process.env.NODE_ENV !== "production");
  const closedMessage =
    admission?.registrationEnabled === false
  ...
  return (
    <div className="auth-minimal-form-wrap">
      ...
      <p className="auth-minimal-terms">
        By continuing, you agree to keep your account secure. Read our{" "}
