...
export default async function DocumentPage(props: {
  ...
  const [documentResult, opportunitiesResult, versionsResult] =
    await Promise.all([
      context.supabase
        .from("documents")
        .select("id, title, kind, status, content, opportunity_id, updated_at")
        .eq("id", id)
  ...
  const opportunities = (opportunitiesResult.data ??
    []) as unknown as LinkedOpportunity[];
    ...
    <div className="page document-editor-page">
      ...
      <header className="page-header">
        <div className="page-header-copy">
          ...
          <p className="page-subtitle">
            {context.project.name} · last saved{" "}
      ...
      {query.saved ? (
        <div className="form-alert success" role="status">
          Document saved as version {versions[0]?.version ?? 1}.
        </div>
      ) : null}
      {versionsResult.error || opportunitiesResult.error ? (
        <div className="form-alert error" role="alert">
          Some document context could not be loaded.
        </div>
      ) : null}
      <form action={updateDocument} className="document-editor">
        ...
        <aside>
          ...
          <div className="field">
            ...
            <SelectField
              ...
              options={["draft", "approved", "submitted", "archived"].map(
                (value) => ({
                  value,
          ...
          <div className="field">
            ...
            <SelectField
              id="opportunityId"
              name="opportunityId"
              defaultValue={document.opportunity_id ?? ""}
              ariaLabel="Linked opportunity"
              options={[
                { value: "", label: "Not attached" },
                ...opportunities.map((opportunity) => ({
                  value: opportunity.id,
                  label: `${formatOpportunityTicket(project.ticket_key, opportunity.reference_number)} · ${opportunity.jobs?.company} · ${opportunity.jobs?.title}`,
                })),
              ]}
            />
          </div>
          <SubmitButton pendingLabel="Saving document…">
            Save new version
          </SubmitButton>
          {versions.length ? (
            <section
              className="document-version-list"
              aria-labelledby="document-versions-heading"
            >
              <h2 id="document-versions-heading">Version history</h2>
