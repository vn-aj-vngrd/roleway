...
export default async function JobsPage({ searchParams }: { searchParams: Promise<{ create?: string; import?: string; error?: string; created?: string }> }) {
  ...
  return (
    <div className="page narrow">
      ...
      {jobs && jobs.length > 0 ? <section className="job-record-list" aria-label="Job inbox">{jobs.map((job) => <article className="job-record" key={job.id}><div className="job-record-main"><div className="muted small">{job.company} · {job.source}</div><h2>{job.title}</h2><p className="page-subtitle">{[job.compensation, job.location, job.remote_policy].filter(Boolean).join(" · ") || "Details not provided"}</p>{job.description ? <p className="job-excerpt">{job.description.slice(0, 220)}{job.description.length > 220 ? "…" : ""}</p> : null}<div className="job-record-actions"><form action={trackJob}><input type="hidden" name="jobId" value={job.id} /><button className="button primary">Track opportunity</button></form><form action={setJobInboxState}><input type="hidden" name="jobId" value={job.id} /><input type="hidden" name="state" value="maybe" /><button className="button secondary">Maybe</button></form><form action={setJobInboxState}><input type="hidden" name="jobId" value={job.id} /><input type="hidden" name="state" value="dismissed" /><button className="button ghost">Dismiss</button></form>{job.source_url ? <a className="button ghost" href={job.source_url} target="_blank" rel="noreferrer"><ExternalLink />Source</a> : null}</div></div><span className="tag">{job.inbox_state === "maybe" ? "Maybe" : "New"}</span></article>)}</section> : null}
    </div>
  );
}
