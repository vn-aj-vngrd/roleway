-- Home replaces the Workspace-scoped Today surface. Preserve existing records while emitting canonical links.
update public.notifications set href = '/home' where href = '/today';
...
begin
  if tg_table_name = 'interviews' then
    ...
    if tg_op = 'INSERT' and new.created_by = 'user' then
      ...
      if enabled then
        insert into public.notifications (user_id, project_id, notification_type, title, href)
        values (new.user_id, new.project_id, 'task', 'Task added · ' || new.title, case when new.opportunity_id is null then '/home' else '/opportunities/' || new.opportunity_id::text end);
      end if;
    end if;
  end if;
  return new;
